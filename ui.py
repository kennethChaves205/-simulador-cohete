...
Interfaz gráfica principal (Tkinter). Contiene:
    ...
    - Franja superior: encabezado de la aplicación.
    - Panel izquierdo: controles, en tarjetas (parámetros, botones, estado).
    - Panel derecho: gráficas en tiempo real (delegado a PlotManager).
...
esa responsabilidad a SimulationController, y solo se encarga de:
    ...
    4. Reenviar cada nuevo estado a PlotManager para graficar.

El estilo visual (colores, tipografía) vive en theme.py para que la UI
y las gráficas de plot_manager.py compartan la misma paleta.
...
class RocketSimulatorApp:
    ...
    def _build_layout(self) -> None:
        """Franja superior + panel izquierdo (controles) y derecho (gráficas)."""
    ...
    def _build_control_panel(self, parent: tk.Widget) -> None:
        ...
        for i, (label_text, var) in enumerate(state_rows):
    ...
    def _build_plot_panel(self, parent: tk.Widget) -> None:
        """Panel derecho: gráficas en tiempo real, dentro de una tarjeta."""
        plot_card = Card(parent, title="📈  Gráficas en tiempo real")
        plot_card.pack(fill=tk.BOTH, expand=True)
        self._plot_manager = PlotManager(plot_card.body)
    ...
    def _handle_state_update(self, state: SimulationState) -> None:
        ...
        (actualizar UI y gráficas) al hilo principal de Tkinter usando
        `after(0, ...)`, que es la forma segura de hacerlo.
        """
